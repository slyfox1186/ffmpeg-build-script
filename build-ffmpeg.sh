...
download_git() {
    local dl_url="$1"
    local dl_file="${2:-"${1##*/}"}"
    local dl_file="${dl_file//\./-}"
    local target_dir="$packages/$dl_file"
    local version

    # Try to get the latest tag
    version=$(git ls-remote --tags "$dl_url" |
    ...
    if [[ -z "$version" ]]; then
        version=$(git ls-remote "$dl_url" | grep HEAD | awk '{print substr($1,1,7)}')
        if [[ -z "$version" ]]; then
            version="unknown"
        fi
    fi
...
pkgs_fn() {
    ...
    pkgs=(
        ...
        autoconf-archive automake autopoint binutils bison build-essential cargo cargo-c ccache
